...
int main() {
    ...
    SuffixArray inv_sa(inv_s);
    
    auto inv_pos = [n](int i) {
        return n-1-i;
    };
    ...
    for(int d = 1; d <= n; d++) {
        for(int i = 0; i+d < n; i+=d) {
            int lcp = sa.get_lcp(i, i+d);
            if(lcp >= d) ans = max(ans, lcp/d+1);
            if(i == 0) continue;
            int lcp2 = inv_sa.get_lcp(inv_pos(i-1), inv_pos(i+d-1));
            if(i-lcp2 + d < n) {
                lcp = sa.get_lcp(i-lcp2, i-lcp2 + d);
                if(lcp >= d) ans = max(ans, lcp/d+1);
            }  
        }
    }
